...
def surrounding_stations(station, line, service):
    ...
    if loop:
        return [(stations[(i-1) % len(stations)], stations[(i+1) % len(stations)]) for i in indices]
    else:
        return [(stations[i-1] if i > 0 else "none", stations[(i+1)] if i < len(stations)-1 else "none") for i in indices]
